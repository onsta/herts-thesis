# Herts thesis

## Project Management

### TODO
- Contact supervisor Imy Khan i.khan9@herts.ac.uk
- Contact Wei Ji about her questions w.1.ji@herts.ac.uk
- Set up initial project skeleton in Jupyter notebook
- Have a first SCP meeting and submit it
- IPP understand the problem domain and the methodology (similar research was it done... how big is the problem, what data do we have), they are looking if there is a solid solution that is achievable, can I go deep and amount of practical work, for the IPP 20 resources is enough 

### Doing

### Questions

### W8ing
- Setup meeting with supervisor as part of SCP (Supervisor Contact Points)
- Write log files x 6 (SCP one per month until November)
- Figure out with the legal team from Herts if I can publish legal MCP, acknowledge
- Be careful about data and get permissions from the source, when publishing from MCP, might need permission from ethiccs

### Done

## Resources (100 papers is a typical for master level)
- https://www.deeplearning.ai/courses/agentic-ai/
- https://www.deeplearning.ai/courses/retrieval-augmented-generation-rag/
- From Retrieval to Reasoning: Leveraging Agentic AI in Czech Supreme Court Case Law Research https://dspace.cuni.cz/handle/20.500.11956/203298

## Contacts
- Module leader w.1.ji@herts.ac.uk
- Supervisor i.khan9@herts.ac.uk

### Feedback IPI 

available from :
https://herts.instructure.com/courses/128587/assignments/403770

This is an interesting project idea. With appropriate level of depth and amount of work, it could be developed to a suitable MSc level project for your study route.

This is essentially an evaluation project. Several points from me:

Will you develop a platform to evaluate different AI techniques or it is only a framework?
Please specify what will be evaluated: performance, capability, or usability?
How will they be evaluated? Manual check? Automated?
What will be the research questions?
What will be the challenge? Any technology will be adopted?
What are the dataset? A set of questions/enquiries? The data need to be justified.
What are your legal bases to retrieve information? Which laws? There are so many so will you focus on one or several?
Any similar research/project in literature? 

These are the questions you will need to address gradually.  

